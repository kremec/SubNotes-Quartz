...
- step transition between neighbouring states, can be infinite possible connections - $N(S)=\{S';\ S\rightarrow S'\}$
...
### Gradient descent
Efficient algorithm for derivative functions
$$
\nabla f(x)=(\frac{\partial f}{\partial x_1},\ ...,\ \frac{\partial f}{\partial x_n})
$$
Maximization / minimization: move in the direction $\nabla f(x)$ / $-\nabla f(x)$
...
Slower decreasing:
- searching larger portion of search space $\rightarrow$ better probability of finding global optimum
- takes more time
