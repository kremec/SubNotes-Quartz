...
![[3-2 Organizacija in management/Organizacija in management podjetja-Image-1.png|400]]
...
- ==strategija fokusa== na določen tržni segment: cilj apelacije na čustva ene specifične skupine ljudi $\rightarrow$ izraba specifične psihološke komponente (npr. izdelek za določeno raso, nacionalnost, ... je zanimiv le specifični skupini)
    - strategija, ko ni dovolj dobrega marketinga, da bi prepričal širše publike o unikatnosti izdelka
- ==strategija diferenciacije==: cilj ljudi prepričati, da izdelek vidijo kot unikatnega za lažjo odločitev nakupa (po visoki ceni) $\rightarrow$ izraba splošne psihološke komponente (npr. prestiž, varnost, ... so vsem zanimivi)
...
![[Organizacija in management podjetja-Image-3.png|400]]
Prednosti:
...
![[Organizacija in management podjetja-Image-4.png|400]]
...
Slabosti:
- povezovanje projektov le na vrhu piramide $\rightarrow$
...
![[Organizacija in management podjetja-Image-6.png|400]]
Prednosti:
...
- vsak ima 2 nadrejena $\rightarrow$ prelaganje odgovornosti, izogibanje ukazom
- dvojna hierarhija $\rightarrow$ otežena identifikacija in odpravljanje organizacijskih težav
#### Procesna organizacijska struktura
![[Organizacija in management podjetja-Image-7.png|400]]
Ista ekipa skozi cel proces $\rightarrow$ enostaven pregled rezultatov in kontrola/pritisk znotraj ekipe $\rightarrow$ večja učinkovitost (agilne metode)
...
- zaposleni se sami odločajo o potrebnem znanju v prihodnosti $\rightarrow$ visoka fleksibilnost in večje zadovoljstvo

Slabosti:
- potrebna visoka samodisciplina za napredek brez nadzora
- težko določanje vlog $\rightarrow$ težko vzpostaviti nagrajevalni sistem in splošna pravila delovanja podjetja
### Situacijska teorija
...
- vsi v ekipi imajo iste vrednote $\rightarrow$ poosebljaj te vrednote
- različne vrednote znotraj skupine $\rightarrow$ splošne/nevtralne vrednote + doseganje rezultatov brez tlačenja podrejenih 
...
- ljudje težko objektivno ocenijo kompetentnost $\rightarrow$ velikokrat neučinkovito
...
![[Organizacija in management podjetja-Image-10.png|300]]
#### Situacijski pristop k vodenju (Fiedler)
Nadgraditev zgornje managerske mreže
==3 situacijski faktorji== $\rightarrow$ ==reakcijo prilagodiš situaciji namesto sebi== (ego vodje velikokrat preprečuje pravilen pristop):
...
![[Organizacija in management podjetja-Image-11.png|500]]
#### Prilagoditev sodelavcem (Hershey, Blanchard)
Nadgraditev managerske mreže
Prilagoditev vodenja glede na motiviranost in kompetentnost sodelavcev $\rightarrow$ 4 stili vodenja:
...
- ==Transformacijski vodja==: njegov odnos do sodelavcev nanje vpliva motivacijsko, da interese kolektiva postavijo pred osebne interese
  Prvi sledilci privabijo nove sledilce $\rightarrow$ oblikuje se čreda z vodjo $\rightarrow$ ni več potrebnih transakcij s posamezniki (kazen je izgon)
